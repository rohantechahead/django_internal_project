...
def UserWishAdd(request):
    ...
    if UserWish.objects.filter(userwish_id=user,title=title, description=description,tag_id= tag_user).exists():
        return Response({"error": "UserWish already created"}, status=status.HTTP_400_BAD_REQUEST)

    connection = UserWish.objects.create(userwish_id=user,title=title, description=description,tag_id= tag_user)
    serializer = UserWishSerializers(connection)
    return Response({"message": "User Wish created successfully", "data": serializer.data}, status=status.HTTP_201_CREATED)

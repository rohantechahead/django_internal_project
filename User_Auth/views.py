...
def user_login(request):
    ...
    try:
        ...
        if timezone.now() <= delete_record.deleted_date + timedelta(days=30):
            ...
            return Response({'error': 'Account cannot be reactivated. The grace period has expired.'},
                            status=status.HTTP_400_BAD_REQUEST)
    except UserDeleteData.DoesNotExist:
        pass
        print("User not marked for deletion.")
...
def user_delete(request):
    try:
        ...
        user.is_login = False
        user.save()
        UserDeleteData.objects.create(user=user, deleted_date=timezone.now()) 
